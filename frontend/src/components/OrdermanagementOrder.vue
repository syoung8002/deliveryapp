    ...
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-actions>
            ...
            <v-btn
                ...
                color="primary"
                text
                @click="openAddReviewAndRating"
            >
                AddReviewAndRating
            </v-btn>
            <v-dialog v-model="addReviewAndRatingDiagram" width="500">
                <AddReviewAndRatingCommand
                    @closeDialog="closeAddReviewAndRating"
                    @addReviewAndRating="addReviewAndRating"
                ></AddReviewAndRatingCommand>
            </v-dialog>
        </v-card-actions>
	...
	async created() {
        ...
        methods: {
            ...
            async addReviewAndRating() {
                try {
                    if(!this.offline){
                        var temp = await axios.post(axios.fixUrl(this.value._links['/addreviewandrating'].href))
                        for(var k in temp.data) this.value[k]=temp.data[k];
                    }
