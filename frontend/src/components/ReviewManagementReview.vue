<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            리뷰 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            리뷰
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            ...
            <String label="주문 ID" v-model="value.orderId" :editMode="editMode" :inputUI="''"/>
